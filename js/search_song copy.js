...
async function getAllSongs(){
  MongoClient.connect(url, {useUnifiedTopology: true} , function(err, client){
    ...
    db.collection(collection_name).find().toArray(function(err, result){
      ...
      console.log(result);
      song_result = result;
      console.log(song_result.values)
      client.close();
      return song_result; 
      });
  });
...
var employeeApp = angular.module("SearchApp",[]);
  employeeApp.controller("songCtrl",function($scope){
    ...
    //doIt(); 
    console.log("Inside Querier")
    //console.log("Inside Querier > Get All Songs  " + main())
    //console.log("Inside Querier > Song Result  " + getAllSongs())
    
    //const song_result_out = await getAllSongs()

    promiseb =  main();

    promiseb.then(function(result) {
      console.log("Promising result "+  stringify(result))
